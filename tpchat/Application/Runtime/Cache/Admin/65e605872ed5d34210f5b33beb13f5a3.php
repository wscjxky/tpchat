...
<table class="table table-hover table-condensed table-striped table-bordered" style="margin-top: 70px ;margin-left: 30px">
    ...
    <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($v["openid"]); ?>">
            ...
            <td ><div class="btn-group" role="group">
                ...
                <?php if($v["agent"] == '非顾问'): ?><a ><button onclick="javascript:agent(this)"data-id="<?php echo ($v["openid"]); ?>" data-act="confirm" type="button"  class="btn btn-success">设为顾问</button></a>
                    <?php else: ?>
                    <a ><button onclick="javascript:agent(this)"data-id="<?php echo ($v["openid"]); ?>" data-act="cancel" type="button"  class="btn btn-danger">取消顾问</button></a><?php endif; ?>
