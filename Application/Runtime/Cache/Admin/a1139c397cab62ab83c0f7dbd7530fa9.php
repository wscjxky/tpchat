						...
						<div class="col-sm-12">
							<table id="list-table" class="table table-bordered table-striped dataTable">
								...
								<?php if(is_array($data)): foreach($data as $k=>$vo): ?><tr role="row" >
										...
										<td><?php echo ($vo["activity_title"]); ?></td>
										...
										<td>1<?php echo ($vo["confirm_url"]); ?>1</td>
										...
										<td>
											<?php if($vo["check_status"] == '正在审核' ): ?><a href="/tpchat/index.php/Admin/Video/video_info?id=<?php echo ($vo["video_id"]); ?>&act=pass" class="btn btn-success">通过</a>
												...
												<?php else: ?>
												<a href="/tpchat/index.php/Admin/Video/video_info?id=<?php echo ($vo["video_id"]); ?>" class="btn btn-info">编辑</a>
                                                <?php if($vo["publish_state"] == 0 ): ?><a> <button data-id="<?php echo ($vo["video_id"]); ?>" onclick="publish(this)" type="button" class="btn btn-success">发布</button></a>
                                                    <?php else: ?>
                                                    <a> <button data-id="<?php echo ($vo["video_id"]); ?>" onclick="publish(this)" type="button" class="btn btn-danger">取消发布</button></a><?php endif; endif; ?>
...
<script>
    function publish(obj) {
        layer.confirm('确认发布？', {
                ...
                $.ajax({
                    type : 'post',
                    url :"/tpchat/index.php/Admin/Video/publish",
                    data : {
